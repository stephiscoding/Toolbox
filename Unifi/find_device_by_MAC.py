...
try:
    ...
    print(
        "Missing argument: please provide the MAC address of the device you're looking for in the format xx:xx:xx:xx:xx:xx."
    )
...
for site in sites:
    ...
    for device in devices:
        if device["mac"].lower() == mac_to_find.lower():
            device_info = api.get_device_info(site, device["mac"])
            print(f"Found {device_info[0]['name']} at site {sites[site]}!")
            if verbosity == "vv":
